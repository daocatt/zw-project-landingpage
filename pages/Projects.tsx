...
import { ExternalLink, QrCode, Search, Calendar } from 'lucide-react';
    ...
    <div className="py-12 bg-slate-50 min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">Our Projects</h1>
          <p className="text-slate-600 max-w-2xl mx-auto">
            A collection of our work, ranging from web applications to mobile tools and experiments.
          </p>
        </div>
    ...
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 flex flex-col h-full group">
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <h3 className="text-xl font-bold text-slate-900 mb-2 leading-tight">
          {project.hasDetail ? (
            <Link to={`/project/${project.id}`} className="hover:text-indigo-600 transition-colors">
              {project.title}
            </Link>
          ) : (
             project.title
        ...
        <div className="pt-4 border-t border-slate-100 flex items-center justify-between mt-auto">
          {project.hasDetail ? (
            ...
            <span className="text-slate-400 text-sm italic">Overview only</span>
          )}
